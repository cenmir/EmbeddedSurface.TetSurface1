...
AllTetEle = [surfh.iel]';
map = [[1:length(surfh)]',AllTetEle];
surfEle = o.CutElements;
N1 = H.neighs;
...
%         %% Viz element
%         xfigure(11212);clf;hold on; axis equal;
%         ele = iel;
%         fele = [4*ele-3;4*ele-2;4*ele-1;4*ele-0];
%         patch(H.XC(H.Faces(fele(:),:)'),H.YC(H.Faces(fele(:),:)'),H.ZC(H.Faces(fele(:),:)'),'w','FaceColor','b');
%         
%         %%
        ielN = unique(N1(iel,:))';
        ielN(isnan(ielN))=[];
...
%         ele(isnan(ele))=[];
%         fele = [4*ele-3;4*ele-2;4*ele-1;4*ele-0];
%         patch(H.XC(H.Faces(fele(:),:)'),H.YC(H.Faces(fele(:),:)'),H.ZC(H.Faces(fele(:),:)'),'w','FaceColor','none');
% 
        neighTri = find(ismember(AllTetEle,neighTet));
%         X = [surfh(neighTri).Xe];
%         
%         x = X(1:3,1:3:end);
%         x = reshape(x,[],1);
...
%             SearchInds = PrevPoints(ismember(PrevPoints,neighPoints));
            if length(PrevPoints) < length(neighPoints)
                SearchInds = PrevPoints;
            else
                SearchInds = neighPoints;
            end
